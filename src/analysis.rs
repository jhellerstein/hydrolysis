...
fn run_nd_pass(graph: &Graph, nodes: &[Node]) -> NdResults {
    ...
    for node in nodes {
        nd_effects
            .entry(node.id.clone())
            .or_insert_with(|| ND_DETERMINISTIC.into());
    }
...
fn check_edge_calm_safe(graph: &Graph, nodes: &[Node], edges: &[Edge], target_edge: &Edge) -> bool {
    use crate::semantics::{Monotonicity, get_node_semantics, is_lattice_type};
...
pub fn run_analysis(ir: &HydroIr) -> AnalysisResult {
    ...
    let overall_deterministic = nd_results
        .nd_effects
        .values()
    ...
    for edge in &ir.edges {
        ...
        let calm = calm_results
            .calm_status
            .get(&edge.id)
            .cloned()
            .unwrap_or_else(|| CALM_SAFE.into());

        edge_analyses.insert(
            edge.id.clone(),
            EdgeAnalysis {
                is_lattice,
                calm,
...
mod tests {
    ...
    fn test_deterministic_graph() {
        ...
        assert_eq!(
            result.node_analyses.get("0").unwrap().nd_effect,
            "Deterministic"
        );
        assert_eq!(
            result.node_analyses.get("1").unwrap().nd_effect,
            "Deterministic"
        );
        assert_eq!(
            result.node_analyses.get("2").unwrap().nd_effect,
            "Deterministic"
        );
    ...
    fn test_nondeterministic_propagation() {
        ...
        assert_eq!(
            result.node_analyses.get("3").unwrap().nd_effect,
            "LocallyNonDet"
        );
    ...
    fn test_calm_safe_network_edge() {
        ...
        let mut edges = vec![make_test_edge("e0", "0", "1", vec!["Network"])];
                ...
                make_sink.prop_flat_map(move |is_sink| {
                    ...
                    node_types_strategy.prop_flat_map(move |mut node_types| {
                        ...
                        let edge_strategy = prop::collection::vec(
                            (0usize..num_nodes)
                                .prop_filter("Create edges to target", move |src| *src < target_idx)
                                .prop_map(move |src| {
    ...
    fn arb_calm_unsafe_graph() -> impl Strategy<Value = (Vec<Node>, Vec<Edge>, String)> {
        ...
        (2usize..8).prop_flat_map(|num_nodes| {
            ...
            critical_edge_target.prop_flat_map(move |target_idx| {
                ...
                (node_types_strategy, make_sink, violation_type).prop_flat_map(
                    move |(mut node_types, is_sink, violation_type)| {
                        // Set target node type
                        if is_sink {
                            node_types[target_idx] = "Sink";
                        }
            ...
            node_types_strategy.prop_flat_map(move |node_types| {
                ...
                let edge_strategy = prop::collection::vec(
                    (
                        ...
                        .prop_map(
                            move |(src, tgt, is_network, has_lattice)| {
